...
class Chain<F, T extends [F, ...any[]]> {
  private _flatValues?: T;

  constructor(readonly _values: Promise<T>) {
    this._values = _values.then(vs => {
      this._flatValues = vs;
      return vs;
    });
  }
  ...
  add<X>(func: (...values: T) => Promise<X> | X): Chain<X, [X, ...T]> {
    return new Chain<X, [X, ...T]>(
      this._values.then(vs => couple(vs, func(...vs)).then(([vls, v]) => [v, ...vls])),
    );
  }
  ...
  with(func: (...values: T) => unknown): Chain<F, T> {
    return new Chain(this._values.then(vs => func(...vs)).then(() => this._values));
  }

  // catch(func: (e: any, ...values: T) => unknown): Chain<F, T> {
  //   const vs = this._flatValues || ([undefined] as unknown as T);
  //   return new Chain(this._values.catch(e => func(e, ...vs)).then(() => this._values));
  // }

  //
  // catch2<X>(func: (e: any, ...values: T) => Promise<X> | X): Chain<X, [X, ...T]> {
  //   const vs = this._flatValues || ([undefined] as unknown as T);
  //   const p = this._values.catch(e => [func(e, ...vs), ...vs]));
  //   return new Chain<X, [X, ...T]>();
  // }

  // .then(vs => couple(vs, func(...vs)).then(([vls, v]) => [v, ...vls])),
 ...
 * ```
 */
export const chain = <V>(value: Promise<V> | V = undefined as any): Chain<V, [V]> =>
  new Chain<V, [V]>(isPromise(value) ? value.then(v => [v]) : Promise.resolve([value]));
